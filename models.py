...
class S2V_QN_1(torch.nn.Module):
    ...
    def forward(self, obs, adj):
        ...
        minibatch_size = obs.shape[0]
        nbr_node = obs.shape[1]
        mu = None  # gets defined in 1st iteration
        for t in range(self.T):
            if t == 0:
                # mu = self.mu_1(xv).clamp(0)
                mu = torch.matmul(obs, self.mu_1).clamp(0)
                # mu.transpose_(1,2)
        ...
        q_1 = self.q_1(torch.matmul(obs.transpose(1, 2), mu)).expand(minibatch_size, nbr_node, self.embed_dim)
        # this does the matrix mult of (the transpose of observation) and mu. and plugs it into q_1
        # expand repeats the vector down a, and to the right b and... back c?
